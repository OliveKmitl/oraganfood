...
import 'package:oraganfood/screens/add_food_manu.dart';
...
class _ListFoodMenuShopState extends State<ListFoodMenuShop> {
  ...
  Widget showContent() {
    return statusRead
        ? Center(
            child: MyStyle().showTitle('ยังไม่มีรายการอาหาร'),
          )
        : ListView.builder(
            itemCount: foodModels.length, //จองหน่วยความจำ
  ...
  Future<Null> confirmDelete(FoodModel foodModel) async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('คุณต้องการลบ ${foodModel.nameFood} จริงหรือ?'),
        children: <Widget>[
          Row(
            ...
            children: <Widget>[
              FlatButton.icon(
                onPressed: () async {
                  ...
                  await Dio().get(urlDelete).then(
                        (value) => readFood(),
                      );
                },
                icon: Icon(Icons.check),
                label: Text('ยืนยันการลบ'),
              ),
              FlatButton.icon(
